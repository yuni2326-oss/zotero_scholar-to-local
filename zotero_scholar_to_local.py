    ...
    try:
        async with await NotebookLMClient.from_storage() as client:
            ...
            try:
                ...
                question = (
                    f"제공된 논문 목록 중 아래 연구 제안과 연관성이 가장 높은 논문 20편을 먼저 선별하고, 선별된 20편만을 근거로 연구 제안의 타당성을 한국어로 평가해주세요.\n\n"
                    f"[연구 제안]\n{proposal}\n\n"
                    f"[평가 항목]\n"
                    f"1. 주요 연구 트렌드와의 관계\n"
                    f"2. 연구 공백(gap) 및 차별성\n"
                    f"3. 타당성 종합 평가\n\n"
                    f"[참고 논문 작성 규칙]\n"
                    f"- 답변 마지막에 '[참고 논문]' 섹션을 작성하세요.\n"
                    f"- 본문에서 논문을 인용할 때 [1], [2] 형식으로 번호를 표기하세요.\n"
                    f"- '[참고 논문]' 섹션의 번호는 본문 인용 번호와 반드시 일치해야 합니다.\n"
                    f"- 형식: [번호] 저자 (연도). 제목."
                )
...
def save_analysis_request(keyword: str, proposal: str, output_dir: Path,
    ...
    instructions = (
        f"이 JSON 파일의 'papers' 필드에 포함된 논문 목록 중 아래 연구 제안과 "
        f"연관성이 가장 높은 논문 20편을 먼저 선별하세요. "
        f"선별된 20편만을 근거로 연구 제안의 타당성을 한국어로 평가해주세요. "
